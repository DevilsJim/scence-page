<template>
  <div class="imageTextBox">
    ...
    <div class="imageTextShow">
      <!--无图-->
      <p v-if="(datumJson.styleType === 'it-text' || datumJson.styleType === 'it-singleImageText' || datumJson.styleType === 'it-imageGalleryText') && datumJson.textContent" class="showText">{{ datumJson.textContent[0].matter }}</p>
      <!--单张图-->
      <div v-if="datumJson.styleType === 'it-singleImage' || datumJson.styleType === 'it-singleImageText'" class="imgShow" :style="{backgroundImage: 'url(' +datumJson.mediaContent[0].matter+ ')'}"></div>
      <!--多张图-->
      <div v-if="datumJson.styleType === 'it-imageGallery' || datumJson.styleType === 'it-imageGalleryText'" class="imagessShow">
        <div class="ltImgBox" :style="{backgroundImage: 'url(' +datumJson.mediaContent[0].matter+ ')'}"></div>
        <div class="rtImgBox">
          <div v-if="datumJson.mediaContent[1]" :style="{backgroundImage: 'url(' +datumJson.mediaContent[1].matter+ ')'}"></div>
          <div v-if="datumJson.mediaContent[2]" class="moreImg" :style="{backgroundImage: 'url(' +datumJson.mediaContent[2].matter+ ')'}">
          	<p v-if="imagessFlagMore" class="moreImgNum">{{ moreNum }}+</p>
          </div>
        </div>
      </div>
    </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      ...
      // 图集
      if ((!this.datumJson.mediaContent) && (this.datumJson.styleType === 'it-singleImage' || this.datumJson.styleType === 'it-singleImageText' || this.datumJson.styleType === 'it-imageGallery' || this.datumJson.styleType === 'it-imageGalleryText')) {
        this.datumJson.mediaContent = this.datumJson.imageContent
      }
      if (this.datumJson.styleType === 'it-imageGallery' || this.datumJson.styleType === 'it-imageGalleryText') {
        var ImgNum = this.datumJson.mediaContent.length
        if (ImgNum > 3) {
          this.imagessFlagMore = true
          this.moreNum = ImgNum - 3
        } else {
          this.imagessFlagMore = false
        }
      }
